			...
			foreach(Room roomB in roomListB) {
				...
				for(int tileIndexA = 0; tileIndexA < roomA.edgeTiles.Count; tileIndexA ++) {
					for(int tileIndexB = 0; tileIndexB < roomB.edgeTiles.Count; tileIndexB ++) {
						...
						int distanceBetweenRooms =(int)(Mathf.Pow(tileA.tileX - tileB.tileX, 2) + Mathf.Pow(tileA.tileX - tileB.tileY, 2)); //tilex, tiley
	...
	class Room : IComparable<Room> {
		...
		public Room(List<Coord> roomTiles, int[,] map) {
			...
			foreach(Coord tile in tiles) {
				for(int x = tile.tileX - 1; x <= tile.tileX + 1; ++x) { //tilex
					for(int y = tile.tileY - 1; y <= tile.tileY + 1; ++y) { //tiley
